...
# library install
mv .download/library/${arch}/library.tar.gz .
if [ $? != 0 ]; then
...
#kube install
mv .download/kube/${arch}/* $buildDir/bin/
if [ $? != 0 ]; then
   echo "====cp kube failed!===="
   exit 1
fi
# registry install
mv .download/registry/${arch}/registry.tar $buildDir/images/registry.tar
if [ $? != 0 ]; then
   echo "====cp registry failed!===="
   exit 1
fi
# cri install
mv .download/containerd/${arch}/cri-containerd-linux.tar.gz $buildDir/cri/
if [ $? != 0 ]; then
   echo "====cp cri failed!===="
   exit 1
fi
# nerdctl install
mv .download/nerdctl/${arch}/nerdctl $buildDir/cri/
if [ $? != 0 ]; then
   echo "====cp nerdctl failed!===="
   exit 1
fi
# shim install
mv .download/shim/${arch}/image-cri-shim $buildDir/cri/
if [ $? != 0 ]; then
   echo "====cp shim failed!===="
   exit 1
fi
# sealctl
mv .download/sealctl/${arch}/image-cri-shim $buildDir/opt/
if [ $? != 0 ]; then
   echo "====cp sealctl failed!===="
   exit 1
fi
# lsof
mv .download/lsof/${arch}/lsof $buildDir/opt/
if [ $? != 0 ]; then
   echo "====cp lsof failed!===="
   exit 1
fi
...
# images
cp -rf  .download/kubeadm/*ImageList   $buildDir/images/shim/
# replace
