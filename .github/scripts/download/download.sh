...
readonly YQ=${yqVersion:-$(
  until curl -sL "https://api.github.com/repos/mikefarah/yq/releases/latest"; do sleep 3; done | grep tarball_url | awk -F\" '{print $(NF-1)}' | awk -F/ '{print $NF}' | cut -dv -f2
)}
...
cd "$ROOT" && {
  ...
  if [[ -n "$sealosPatch" ]]; then
    ...
    until curl -sL "https://github.com/labring/sealos/releases/download/v$SEALOS/sealos_${SEALOS}_linux_amd64.tar.gz"; do sleep 3; done |
      tar -zx sealos
  fi
  until curl -sLo "yq" "https://github.com/mikefarah/yq/releases/download/v$YQ/yq_linux_amd64"; do sleep 3; done
}
