...
//WatchFileSize is
func (ss *SSH) LoggerFileSize(filename string, size int) {
	t := time.NewTicker(3 * time.Second) //every 3s check file
	defer t.Stop()
	for {
		select {
		case <-t.C:
			length := ss.CmdToString("ls -l "+filename+" | awk '{print $5}'", "")
			...
			logger.Info("[ssh][%s]transfer total size is: %.2f%s", ss.Host, value, "MB")
		}
	}
}

//RemoteFileExist is
func (ss *SSH) IsFileExist(remoteFilePath string) bool {
	// if remote file is
	// ls -l | grep aa | wc -l
	...
	data := ss.CmdToString(remoteFileCommand, " ")
	count, err := strconv.Atoi(strings.TrimSpace(data))
	defer func() {
		if r := recover(); r != nil {
			logger.Error("[ssh][%s]RemoteFileExist:%s", ss.Host, err)
		}
	}()
	if err != nil {
		panic(1)
