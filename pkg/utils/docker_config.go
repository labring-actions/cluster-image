...
func DockerConfig() (*DockerInfo, error) {
	authFile := DefaultRegistryAuthConfigDir()
	if !FileExist(authFile) {
		if err := os.MkdirAll(filepath.Dir(authFile), 0755); err != nil {
			return nil, err
		}
		return &DockerInfo{Auths: map[string]AuthItem{}}, AtomicWriteFile(authFile, []byte("{\"auths\":{}}"), 0644)
	}
...
func SetDockerConfig(hostname, username, password string) error {
	...
	if err := AtomicWriteFile(authFile, data, 0644); err != nil {
