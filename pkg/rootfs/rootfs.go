...
func (b *build) kubePackage() error {
	...
	for _, version := range b.KubeVersions {
		...
		logger.Info("script build cluster image %s successfully!", fmt.Sprintf("%s:%s", strings.Join([]string{vars.Run.RegistryDomain, vars.Run.RegistryRepo, "kubernetes"}, "/"), version))
	}
	if len(errs) > 0 {
...
func (b *build) appPackage() error {
	...
	logger.Info("script build cluster image %s successfully!", fmt.Sprintf("%s:%s", strings.Join([]string{vars.Run.RegistryDomain, vars.Run.RegistryRepo, b.APPType}, "/"), b.APPVersion))
	return nil
}
