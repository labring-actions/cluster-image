...
function init(){
  ...
  wget -qP opt/ https://github.com/argoproj/argo-rollouts/releases/download/v1.5.1/kubectl-argo-rollouts-linux-amd64
  mv opt/kubectl-argo-rollouts-linux-amd64 opt/kubectl-argo-rollouts
  chmod +x opt/kubectl-argo-rollouts
  # parse kubectl-argo-rollouts image
  cat >charts/argo-rollouts.values.yaml<<EOF
dashboard:
  enabled: true
EOF
}
