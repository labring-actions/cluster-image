...
POD_CIDR=${POD_CIDR:-"100.64.0.0/10"}
POD_GATEWAY=${POD_GATEWAY:-"100.64.0.1"}
SVC_CIDR=${SVC_CIDR:-"10.96.0.0/22"}
JOIN_CIDR=${JOIN_CIDR:-"20.65.0.0/16"}
PINGER_EXTERNAL_ADDRESS=${PINGER_EXTERNAL_ADDRESS:-"114.114.114.114"}
PINGER_EXTERNAL_DOMAIN=${PINGER_EXTERNAL_DOMAIN:-"alauda.cn"}
SVC_YAML_IPFAMILYPOLICY=${SVC_YAML_IPFAMILYPOLICY:-}
if [ "$IPV6" = "true" ]; then
...
LABEL=${LABEL:-"node-role.kubernetes.io/control-plane"}
DEPRECATED_LABEL=${DEPRECATED_LABEL:-"node-role.kubernetes.io/master"}
NETWORK_TYPE=${NETWORK_TYPE:-"geneve"}
TUNNEL_TYPE=${TUNNEL_TYPE:-"geneve"}
POD_NIC_TYPE=${POD_NIC_TYPE:-"veth-pair"}
POD_DEFAULT_FIP_TYPE=${POD_DEFAULT_FIP_TYPE:-}

# VLAN Config only take effect when NETWORK_TYPE is vlan
PROVIDER_NAME=${PROVIDER_NAME:-"provider"}
VLAN_INTERFACE_NAME=${VLAN_INTERFACE_NAME:-}
VLAN_NAME=${VLAN_NAME:-"ovn-vlan"}
VLAN_ID=${VLAN_ID:-"100"}
