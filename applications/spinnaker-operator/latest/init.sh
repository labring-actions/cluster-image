...
init_dir() {
    ...
    rm -rf "${OPT_DIR}" "${CHARTS_DIR}" "${IMAGES_DIR}" "${ETC_DIR}"
    mkdir -p "${MANIFESTS_DIR}" "${IMAGES_DIR}" "${ETC_DIR}"
}
...
download_spinnaker_operator_manifests() {
    ...
    wget -qO- "${DOWNLOAD_URL}" | tar -zx -C "${ETC_DIR}"
}
...
download_spinnaker_kustomize_patches() {
    ...
    wget -qO- "${DOWNLOAD_URL}" | tar -zx -C "${ETC_DIR}"
    rm -rf ${ETC_DIR}/spinnaker-kustomize-patches-master/kustomization.yml
    cp -rf ${ETC_DIR}/spinnaker-kustomize-patches-master/recipes/kustomization-all.yml ${ETC_DIR}/spinnaker-kustomize-patches-master/kustomization.yml
}
...
download_spinnaker_images() {
    ...
    for i in {1..10}; do
        status=$(docker exec halyard hal --ready &> /dev/null)
        if [ $? -eq 0 ]; then
            echo "halyard ready"
            break
        else
    ...
    local latestSpinnaker=$(docker exec halyard hal version latest -q)
    local spinnaker_bom="${ETC_DIR}/spinnaker_bom.yaml"
    docker exec halyard hal version bom ${latestSpinnaker} -q -o yaml > "${spinnaker_bom}"
    yq -r '.services | to_entries | .[] | env(spinnaker_dockerRegistry) + .key + ":" + .value.version' "${spinnaker_bom}" > ${IMAGES_DIR}/spinnaker_images.txt

    local sinnaker_deployment="${ETC_DIR}/deploy/operator/cluster/deployment.yaml"
    local minio_deployment="${ETC_DIR}/spinnaker-kustomize-patches-master/core/persistence/in-cluster/minio.yml"
    ...
    sed -i "s|version:.*|version: ${latestSpinnaker}|g" ${MANIFESTS_DIR}/spinnakerService.yaml.tmpl
}
