...
if ! [ -x /usr/bin/crictl ]; then
  ...
  if command_exists nvidia-smi ; then
    sed -i "s#__options__#BinaryName = \"nvidia-container-runtime\"#g" /etc/containerd/config.toml
    tar -zxf ../cri/nvidia-container-runtime.tar.gz -C /
    mkdir -p /opt/containerd && tar -zxf ../cri/lib64/nvidia-lib.tar.gz -C /opt/containerd
  else
